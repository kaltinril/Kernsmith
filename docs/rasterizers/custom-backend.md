...
Since `RasterizerBackend` is an enum in the core assembly, third-party backends use a numeric cast. Pick a value of 100 or higher to avoid collisions with built-in backends (values 0-4 are reserved by the core enum: FreeType=0, Gdi=1, DirectWrite=2, StbTrueType=3, and 4 is held for future use):
