...
```
bmfontier generate -f MyFont.ttf -s 32
```

Generate bold + italic at 48px and save as XML:

```
bmfontier generate -f MyFont.ttf -s 48 -b -i --format xml
```

Generate with extended Latin characters and a 2px outline:

```
bmfontier generate -f MyFont.ttf -s 24 -c latin --outline 2
```

Use a system-installed font instead of a file:

```
bmfontier generate --system-font "Arial" -s 32
```
...
bmfontier generate -f font.ttf -s 32

# Extended ASCII (0x00-0xFF)
bmfontier generate -f font.ttf -s 32 -c extended

# Latin characters
bmfontier generate -f font.ttf -s 32 -c latin

# Specific characters only
bmfontier generate -f font.ttf -s 32 --chars "ABCabc0123"

# Characters from a file
bmfontier generate -f font.ttf -s 32 --chars-file my-chars.txt

# Unicode range (Basic Latin + Latin-1 Supplement)
bmfontier generate -f font.ttf -s 32 --range 0020-007E --range 00A0-00FF
```
...
bmfontier generate -f font.ttf -s 32 --autofit

# Large atlas with per-side padding
bmfontier generate -f font.ttf -s 64 --max-texture 2048 --padding 2,4,2,4

# Non-power-of-two, skyline packer
bmfontier generate -f font.ttf -s 32 --no-pot --packer skyline
```
...
bmfontier generate -f RobotoFlex.ttf -s 32 --axis wght=700 --axis wdth=90

# Use a named instance
bmfontier generate -f RobotoFlex.ttf -s 32 --instance "Bold"
```
...
bmfontier generate -f font.ttf -s 32 --format xml --texture-format tga

# Output to a specific directory
bmfontier generate -f font.ttf -s 32 -o output/myfont
```
...
```
# Convert text to XML
bmfontier convert myfont.fnt -o myfont.xml

# Convert to binary with explicit format
bmfontier convert myfont.fnt -o output/myfont.fnt --format binary
```
...
```
bmfontier list-fonts [--filter <pattern>] [--json]
```
...
```
bmfontier info <path> [--json]
```

```
# Human-readable
bmfontier info Roboto-Regular.ttf
...
bmfontier generate -f font.ttf -s 32 --outline 2

# 3px red outline
bmfontier generate -f font.ttf -s 32 --outline 3,FF0000
```
...
bmfontier generate -f font.ttf -s 32 --gradient FFFFFF,888888

# Two-argument form
bmfontier generate -f font.ttf -s 32 --gradient FFFFFF 888888

# Rotated gradient (45 degrees)
bmfontier generate -f font.ttf -s 32 --gradient FF0000,0000FF --gradient-angle 45

# Shift the gradient midpoint (0.0-1.0, default 0.5)
bmfontier generate -f font.ttf -s 32 --gradient FFFFFF,000000 --gradient-midpoint 0.3
```
...
```
# Simple 2px shadow
bmfontier generate -f font.ttf -s 32 --shadow 2,2

# Black shadow with blur
bmfontier generate -f font.ttf -s 32 --shadow 2,2,000000,1

# Colored shadow, no blur
bmfontier generate -f font.ttf -s 32 --shadow 1,1,444444
```
...
```
# Outline + gradient
bmfontier generate -f font.ttf -s 48 --outline 2,000000 --gradient FFFFFF,AAAAAA

# Outline + shadow
bmfontier generate -f font.ttf -s 48 --outline 2 --shadow 2,2,000000,1

# All three
bmfontier generate -f font.ttf -s 48 --outline 2,000000 --gradient FFFFFF,888888 --shadow 2,3,000000,2
```
...
bmfontier generate --config mygame-font.bmfc

# Config + CLI overrides (CLI flags take precedence)
bmfontier generate --config mygame-font.bmfc -s 48 --format xml

# Save current settings to a config file
bmfontier generate -f font.ttf -s 32 --outline 2 --save-config my-settings.bmfc

# Load, tweak, and re-save
bmfontier generate --config old.bmfc -s 64 --save-config updated.bmfc
```
...
```
bmfontier generate -f font.ttf -s 32 --super-sample 2
bmfontier generate -f font.ttf -s 16 --super-sample 4
```
...
```
bmfontier generate -f font.ttf -s 32 --sdf --padding 4
```
...
Set variation axes directly or use named instances. Use `bmfontier info` to discover available axes and instances.

```
# Check what axes are available
bmfontier info RobotoFlex.ttf
...
```
bmfontier generate -f font.ttf -s 32 --channel-pack
```
...
bmfontier generate -f font.ttf -s 32 --chars "0123456789.,:-"

# Characters from a file (one per line or all on one line)
bmfontier generate -f font.ttf -s 32 --chars-file game-strings.txt

# Specific Unicode range (CJK Unified Ideographs subset)
bmfontier generate -f font.ttf -s 24 --range 4E00-4FFF
```
...
```
bmfontier generate -f NotoColorEmoji.ttf -s 32 --color-font
bmfontier generate -f NotoColorEmoji.ttf -s 32 --color-font --color-palette 1
```
...
```
bmfontier generate -f font.ttf -s 32 --outline 2 -c latin --dry-run
```
