...
| ID  | Task                    | Depends On       | Description                                                                                        | Docs to Read                                                                  |
|-----|-------------------------|-----------------|----------------------------------------------------------------------------------------------------|--------------------------------------------------------------------|
| 13A | **WOFF decompression**  | Phase 2 complete | Implement WOFF/WOFF2 header parsing and zlib/Brotli decompression to extract inner TTF/OTF          | reference/other-font-formats-reference.md (WOFF/WOFF2 sections)    |
| 13B | **Color font support**  | Phase 2 complete | COLRv0/CPAL layer rendering, sbix bitmap extraction, CBDT/CBLC bitmap extraction                    | reference/other-font-formats-reference.md (Color Fonts section)    |
| 13C | **Font subsetting**     | Phase 2 complete | Strip unused glyphs from font data before processing (reduces memory for large CJK fonts)           | New research needed                                                |

### Group N — Advanced Atlas Features

| ID  | Task                    | Depends On       | Description                                                                       | Docs to Read                                                                                                         |
|-----|-------------------------|-----------------|-----------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------|
| 14A | **Channel packing**     | Phase 2 complete | Pack monochrome glyphs into individual RGBA channels for 4x density                | reference/texture-packing-reference.md (Channel Packing section), reference/bmfont-format-reference.md (Channel section) |

### Group O — Tooling + Publishing

| ID  | Task                        | Depends On                       | Description                                                                                          | Docs to Read                 |
|-----|-----------------------------|----------------------------------|------------------------------------------------------------------------------------------------------|------------------------------|
| 15A | **Reference CLI tool**      | Phase 2 complete                 | Simple CLI wrapper: `bmfontier generate -f font.ttf -s 32 -o output/`                                | New plan doc needed          |
| 15B | **Performance benchmarks**  | Phase 2 complete                 | Benchmark suite: measure time/memory for ASCII set at various sizes, compare MaxRects vs Skyline      | New plan doc needed          |
| 15C | **NuGet publishing**        | Phase 2 complete, license decided | Configure CI for NuGet pack + push, README, package icon                                             | plan-project-structure.md    |

### Group P — Phase 3 Tests

| ID  | Task                        | Depends On | Description                                                              | Docs to Read     |
|-----|-----------------------------|-----------|--------------------------------------------------------------------------|-----------------|
| 16A | **Tests: WOFF**             | 13A       | Load WOFF/WOFF2 font, verify decompression and generation                 | plan-testing.md |
| 16B | **Tests: channel packing**  | 14A       | Verify channel-packed atlas has glyphs in correct channels                 | plan-testing.md |
| 16C | **Tests: CLI**              | 15A       | End-to-end CLI invocation tests                                           | plan-testing.md |
