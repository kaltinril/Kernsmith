# bmfontier CLI -- Full-Featured BMFont Generator

> Plan for a production-ready CLI tool that replaces AngelCode's BMFont.exe.
> This is a separate project (`Bmfontier.Cli`) distributed as a .NET global tool.
...
dotnet tool install -g Bmfontier.Cli

# Or run without installing
dotnet tool run bmfontier -- generate -f font.ttf -s 32
```
...
bmfontier generate -f opensans.ttf -s 64 --sdf --padding 8

# Gradient gold text
bmfontier generate -f impact.ttf -s 72 --gradient FFD700 DC143C

# Variable font with specific weight
bmfontier generate -f "RobotoFlex.ttf" -s 32 --axis wght=700

# System font, XML output, Cyrillic charset
bmfontier generate --system-font "Arial" -s 24 --format xml --range 0020-007E --range 0400-04FF

# Channel-packed atlas for maximum density
bmfontier generate -f font.ttf -s 16 --channel-pack --max-texture 512

# From a config file
bmfontier generate --config mygame-fonts.bmfc

# Save settings for reuse
bmfontier generate -f font.ttf -s 32 --outline 2 --save-config mypreset.bmfc
```
...
```ini
# bmfontier configuration file
# Generated by: bmfontier generate --save-config
...
bmfontier generate -f font.ttf -s 32 -c ascii --chars "@#$" --range 0400-04FF
```
...
cli/
  Bmfontier.Cli/
    Bmfontier.Cli.csproj        # .NET global tool package
...
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    ...
    <ToolCommandName>bmfontier</ToolCommandName>
    <PackageId>Bmfontier.Cli</PackageId>
  ...
  <ItemGroup>
    <PackageReference Include="Bmfontier" Version="0.1.0" />
  </ItemGroup>
</Project>
```
...
dotnet pack cli/Bmfontier.Cli -c Release

# Test locally
dotnet tool install -g --add-source ./cli/Bmfontier.Cli/nupkg Bmfontier.Cli

# Publish to NuGet
dotnet nuget push cli/Bmfontier.Cli/nupkg/Bmfontier.Cli.0.1.0.nupkg -k <api-key> -s https://api.nuget.org/v3/index.json
```

---

## Comparison: bmfontier CLI vs BMFont.exe

| Feature | BMFont.exe | bmfontier CLI |
|---------|-----------|---------------|
...
### Phase A -- Core CLI (replace current samples/Bmfontier.Cli)

| Task | Description |
|------|-------------|
| A1 | Set up `cli/Bmfontier.Cli` project with .NET global tool config |
...
Create `cli/Bmfontier.Cli/Bmfontier.Cli.csproj` with the NuGet tool configuration shown above.
Add a project reference to `../../src/Bmfontier/Bmfontier.csproj` for local development (the
...
Examples:
  bmfontier generate -f arial.ttf -s 32
  bmfontier generate -f roboto.ttf -s 48 -b --outline 2 --format xml
...
1. `dotnet build cli/Bmfontier.Cli -c Release`
2. `dotnet pack cli/Bmfontier.Cli -c Release -o ./nupkg`
3. `dotnet nuget push ./nupkg/Bmfontier.Cli.*.nupkg -k $NUGET_API_KEY -s https://api.nuget.org/v3/index.json`
...
- `Bmfontier` NuGet package (the library) -- all font generation logic
...
This section documents every `Bmfontier` library API that the CLI depends on, so implementers
