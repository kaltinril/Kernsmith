...
- Supersampling -- renders at `size * aa` via supersized HFONT, downscales bitmap by averaging aa x aa blocks, divides metrics by aa
...
Remaining gaps (xoffset +/-1 systematic, yoffset +/-1-3, Bell MT edge cases, Bahnschrift missing pairs, atlas channel config) have been moved to Phase 99 for future investigation.
...
- xoffset +/-1 systematic -- architectural limitation (different GDI rendering paths)
- Bahnschrift 47-63 missing kerning pairs -- unknown source in BMFont pipeline
