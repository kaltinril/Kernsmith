...
>   - **Phases B and C intentionally NOT done -- carried forward as future work**: the atlas page buffer (`AtlasPage.PixelData`) and per-glyph `RasterizedGlyph.BitmapData` escape into the public API. Pooling them safely requires an `IDisposable`/ownership API redesign; without it, returning a buffer to the pool risks use-after-return on caller-held data. Deferred deliberately, not forgotten. **Carried forward to [Phase 300 -- Deferred Performance Work](../phase-300-deferred-performance.md).**
