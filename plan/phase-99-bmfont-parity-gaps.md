...
> **The harness is deterministic -- do not dismiss its diffs.** Verified 2026-07-29 with two controlled runs: a no-op branch (identical source on both sides) and a branch adding only a new `.bmfc` config (baseline generated 128 fonts, branch 133). **Both came back completely clean** -- 4/4 hardcoded comparisons, 24/24 per-font, 170/170 FNT files identical. Treat a reported diff as real until proven otherwise.
>
> **One unexplained observation, recorded for future reference**: on 2026-07-28 a run on a branch touching only `.bmfc` reader/writer code reported 42 differing pixels across `Font18Arial`, `Font24Arial` and `plain-nosmoothing` (roughly half in the harness's own GDI+ title label, drawn at `GenerateAll/Program.cs:519` with `new Font("Arial", 11, FontStyle.Italic)`; the rest ±8-11 alpha on glyph edges, beyond what `--tolerance 1` absorbs). A `git worktree` cross-check at the time showed **main vs main produced the same 41/37/36-pixel diff** while **main vs branch produced 0**, proving the change under test was not the cause. It has **not reproduced since**, including under the two controlled runs above, and the "branch adds a new config" theory was explicitly tested and disproven. Cause unknown; assume environmental until it recurs.
>
> **If a diff looks suspicious, confirm rather than assume** (either direction): generate `main` into a separate `git worktree` and diff main-vs-main. That distinguishes a real regression from environmental noise without guessing.
>
> **Coverage limit (durable)**: `diff_comparisons.py` only discovers `*.png` and `*.fnt` -- it never diffs `.bmfc`, so the harness gives **zero** coverage of config reader/writer changes regardless of what else it reports.
