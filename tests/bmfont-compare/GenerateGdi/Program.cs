...
// Force the GDI rasterizer assembly to load so its ModuleInitializer registers the backend.
...
var outputDir = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "gum-gdi"));
...
foreach (var bmfcPath in bmfcFiles)
{
    ...
    try
    {
        var result = BmFont.Builder()
            .FromConfig(bmfcPath)
            .WithRasterizer(new KernSmith.Rasterizers.Gdi.GdiRasterizer())
            .Build();
